...
def get_window_text_windows():
    if (sys_wrapper.is_windows()):
        returning = ""
        window = win32gui.GetForegroundWindow()
        print("window: %s" % window)
        if (window != 0):
            if (win32gui.GetClassName(window) == 'CabinetWClass'): # the main explorer window
                print("class: %s" % win32gui.GetClassName(window))
                print("text: %s " %win32gui.GetWindowText(window))
                children = list(set(searchChildWindows(window)))
                addr_edit = None
                file_view = None
                for child in children:
                    childText = getEditText(child)
                    #logz(f"childText:{childText}")
                    if (childText):
                        returning += f"{childText}\n"
